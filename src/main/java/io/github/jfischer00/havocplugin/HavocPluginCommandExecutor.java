			...
			boolean playerFound = false;
				...
				for (Player target : Bukkit.getServer().getOnlinePlayers()) {
					if (target.getName().equalsIgnoreCase(args[0])) {
						...
						{
							Bukkit.getServer().dispatchCommand(Bukkit.getServer().getConsoleSender(), "spawnmob ghast,ghast 5 " + target.getName());							
							target.sendMessage(ChatColor.YELLOW + "Beware of fireballs!");
							return true;
						}
						else if (args[1].equalsIgnoreCase("creepers"))
						{
							Bukkit.getServer().dispatchCommand(Bukkit.getServer().getConsoleSender(), "spawnmob creeper,creeper 5 " + target.getName());							
							target.sendMessage(ChatColor.YELLOW + "Watch out for explosions!!");
							return true;
						}
						else if (args[1].equalsIgnoreCase("fire"))
						{
							target.setFireTicks(1200);
							target.sendMessage(ChatColor.YELLOW + "Ouch, Hot! Better find some water.");
							return true;
						}
						else if (args[1].equalsIgnoreCase("kill"))
						{
							...
							target.setHealth(0);
							target.sendMessage(ChatColor.YELLOW + "BOOM! What happened?");
							return true;
						}
						else if (args[1].equalsIgnoreCase("lightning"))
						{
							target.getWorld().strikeLightning(target.getLocation());
							target.sendMessage(ChatColor.YELLOW + "FLASH! That's hot.");
							return true;
						}
						else if (args[1].equalsIgnoreCase("hide"))
						{
							for (Player hidden : Bukkit.getOnlinePlayers())
							{
								if (!hidden.getName().equalsIgnoreCase(args[0]))
								{
									...
									target.sendMessage(ChatColor.YELLOW + "Where did everyone go?");
									return true;
								}
							}
						}
